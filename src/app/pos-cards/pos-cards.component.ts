...
import {
  ...
  OrderResponse,
  OrderSaleDailyReport,
  OrderSaleReportResponse,
  OrderSaveResponse,
  OrderSearch,
  Orders,
...
@Component({
  selector: 'app-pos-cards',
  templateUrl: './pos-cards.component.html',
  styleUrls: ['./pos-cards.component.scss']
})
export class PosCardsComponent implements OnInit, OnDestroy {
  ...
  pickupCartDataList: CartHold = new CartHold();
  ...
  ngOnInit(): void {
    ...
    let holdData = localStorage.getItem('pickupCart');
    ...
    if (holdData) {
      this.pickupCartDataList = JSON.parse(holdData);
      //@@COMMENTED on Mar19
      // this.customer.firstName = this.pickupCartDataList.customer.firstName;
      // this.customer.email = this.pickupCartDataList.customer.email;
      // this.customer.phone1 = this.pickupCartDataList.customer.phone1;
      ...
      //this.priceSummary.grandTotal = this.pickupCartDataList.subTotal;
  ...
  ngAfterViewInit(): void {
    let len = this.pickupCartDataList.product.length - 1;
  ...
  discountAllNotFocus() {
    for (let i = 0; this.pickupCartDataList.product.length; i++) {
  ...
  addToCart(productView: ProductView, quantity: number = 1) {
    if (productView) {
      ...
      let localCartData = localStorage.getItem('pickupCart');
      if (localCartData) {
        ...
        rcvdProduct.shipping = this.pickupCartDataList.shipping;
        rcvdProduct.subTotal = this.pickupCartDataList.subTotal;
        rcvdProduct.taxes = this.pickupCartDataList.taxes;
        rcvdProduct.transactionId = 0;
        rcvdProduct.total = this.pickupCartDataList.total;
      ...
      this.productService.pickupAddToCart(rcvdProduct);
      //this.cartDataList1.push(rcvdProduct);


      // Add the product directly to the cart
      // this.productService.pickupAddToCart(cartHold);
  ...
  addItemsToCart(productView: ProductView) {
    ...
    if (localCartData) {
      ...
      if (cartData.product.length > 0) {
        ...
        if (!found) {
          ...
          this.pickupCartDataList = cartData;
          this.calculateDiscount(this.pickupCartDataList.product[foundRow].discount, foundRow);
    ...
    this.pickupCartDataList = cartData; //Assign current local cartData
    //Finally, set all variables and focus on Discount
    let count = this.pickupCartDataList.product.length - 1;
    ...
    this.pickupCartDataList.discountPercentage = Number(this.customer?.discountPercentage) || 0;
    ...
    this.priceCalculationTotal();
    //Save again
    localStorage.setItem('pickupCart', JSON.stringify(this.pickupCartDataList));
  ...
  openRetrieveSalePopup() {
    ...
    if (pickupCart) {
      ...
      let cartDataArray = JSON.parse(pickupCart);
  ...
  openCashModal() {
    let pickupCart = localStorage.getItem('pickupCart')
    if (pickupCart === undefined || pickupCart === '' || pickupCart === null || pickupCart.length === 105 || pickupCart.length === 0) {
    ...
    if (pickupCart != null || pickupCart != undefined) {
      this.pickupCartDataList = JSON.parse(pickupCart);
      if (this.pickupCartDataList.product.length === 0) {
        Swal.fire('WARNING', 'Cart is Empty', 'warning');
        return;
      }
      else {
        ...
        if (this.pickupCartDataList.total < 0) {
          ...
          resultInput.value = '' + this.pickupCartDataList.total; //.toFixed(2);
        }
        else {
  ...
  checkHoldSale() {
    ...
    let pickupCart = localStorage.getItem('pickupCart');
    //pickupCart = always one object of cartHold type
  ...
  holdSale() {
    ...
    this.pickupCartDataList.customer.firstName = this.customer.firstName;
    this.pickupCartDataList.customer.email = this.customer.email;
    this.pickupCartDataList.customer.phone1 = this.customer.phone1;
    ...
    if (pickupCart) {
      ...
      if (holdCartListObj === null) {
        ...
        let currentHoldData: CartHold = JSON.parse(pickupCart);
        ...
        let localCartObj: CartHold = JSON.parse(pickupCart);
      ...
      localStorage.removeItem('pickupCart');
      window.location.reload();
  ...
  qtyChange(row: number) {
    ...
    if (qtyInput != null || qtyInput != undefined) {
      ...
      if (localCartData) {
        ...
        localStorage.setItem('pickupCart', JSON.stringify(qtydata));

        this.pickupCartDataList = qtydata;
  ...
  /* *************************************************************** */
  calculateQtyTotal(pickupCartDataList: CartHold) {
    this.pickupCartDataList.totalQty = 0;
    if (pickupCartDataList !== undefined) {
      if (pickupCartDataList.product.length > 0) {
        for (let i = 0; i < pickupCartDataList.product.length; i++) {
          this.pickupCartDataList.totalQty = this.pickupCartDataList.totalQty + Number(pickupCartDataList.product[i].quantity);
        }
        this.pickupCartDataList.totalItems = pickupCartDataList.product.length;
      }
    }
  }
  ...
  removeItem(orderItem: any, index: any) {
    let itemToRemove = this.pickupCartDataList.product[index];
    //let details = this.pickupCartDataList.product[index]?.productDetails;
    //this.pickupCartDataList.product.splice(index, 1);

    this.pickupCartDataList.product = this.pickupCartDataList.product.filter(item => item !== itemToRemove);

    this.priceCalculationTotal();

    if (this.pickupCartDataList.product.length === 0) {
      this.pickupCartDataList.discount = 0;
      this.totalDiscount = 0;
      this.pickupCartDataList.discountPercentage = 0;
      this.totalDiscountPercentage = 0;
      this.pickupCartDataList.taxes = 0;
      this.pickupCartDataList.taxesPercentage = 0;
      this.pickupCartDataList.totalItems = 0;
      this.pickupCartDataList.totalQty = 0;
  ...
  /* *********** CHECKOUT *************** */
  //Steps: Customer Save, if it is a new Customer or not default Customer. Or Use Default customer
  //Step2: 
  ...
  createCheckout(source: any) {
    ...
    if (this.payment.paymentMethod === 'CASH') {
        ...
        if (this.pickupCartDataList.total < 0) {
          Swal.fire({
            ...
            if (!response.value) {
              ...
              this.payment.remainingBalance = (this.pickupCartDataList.total);
              this.payment.discount = this.pickupCartDataList.discount;
      ...
      else if (this.result < this.pickupCartDataList.total) {
      ...
      this.result = this.pickupCartDataList.total;
      if (this.result < this.pickupCartDataList.total) {
  ...
  async createSaleOrder() {
    ...
    this.payment.discount = this.pickupCartDataList.discount;
    if (this.showTaxFlag) {
      this.payment.taxesAmount = this.pickupCartDataList.taxes;
    }
    else {
      this.payment.taxesAmount = 0;
    }

    this.payment.totalAmount = this.pickupCartDataList.total;
    ...
    this.commonCheckOut(this.pickupCartDataList, customer);
  ...
  getOrderAmount(order: Orders) {
    ...
    let orderAmount = 0;
    let discountVal = 0;
    this.pickupCartDataList.product.forEach((items) => {
  ...
  commonCheckOut(pickupCart: any, customer: Customer) {
    ...
    if (pickupCart) {
      ...
      //order.shippingHandling = this.pickupCartDataList.delivery.toFixed(2);
      ...
      });//this.pickupCartDataList?.forEach((items)=>
                ...
                .then((result) => {
                  {
                    ...
                    //printThermal(customer: Customer, payment: Payment, pickupCartDataList: CartHold, customerBalance: number, invoiceNumber: any)
                    let dinInFlag = false;
                    this.printService.printThermalRestaurant(customer, this.payment, this.pickupCartDataList,
  ...
  weightChange(index: number) {
    ...
    this.pickupCartDataList.product[index].quantity = qty.value;
  ...
  calculateFooterBalance(): void {
    ...
    this.pickupCartDataList.discount = cashDiscount;
    //let invoiceTotal = Number(this.priceSummary.total) || 0;


    if (this.showTaxFlag) {
      this.customerBalance = (this.pickupCartDataList.subTotal - cashDiscount + this.pickupCartDataList.taxes);
    }
    else {
      this.customerBalance = (this.pickupCartDataList.subTotal - cashDiscount);
    }

    this.pickupCartDataList.total = this.customerBalance;
    this.pickupCartDataList.total = Number(this.pickupCartDataList.total);
  ...
  calculateBalance(): void {
    ...
    this.customerBalance = cashPaid - (this.pickupCartDataList.total);
  ...
  showHoldSaleDetails(cartHold: CartHold): void {
    //let data = localStorage.getItem('pickupCart');

    localStorage.setItem('pickupCart', JSON.stringify(cartHold));
  ...
  priceChange(row: number) {
    ...
    this.pickupCartDataList.product[row].price = priceInput.value;
    ...
    if (localCartData) {
      ...
      localStorage.setItem('pickupCart', JSON.stringify(discountData))
    }
  ...
  notesChange(row: number, notes: any) {
    this.pickupCartDataList.product[row].notes = notes;
  ...
  discountChange(row: number) {
    ...
    const item = this.pickupCartDataList.product[row];
  ...
  selectSearchProduct(product: ProductView) {
    ...
    if (localCartData) {
      ...
      localStorage.setItem('pickupCart', JSON.stringify(cartData));
    }
    else {
      product.quantity = 1;
      product.loginId = this.selectedAgent?.loginId;
      ...
      this.pickupCartDataList.product.push(product);
      localStorage.setItem('pickupCart', JSON.stringify(this.pickupCartDataList));
    }
*/
    this.closeProductSearchPopup();
...
@media print {
  ...
  priceCalculationPerRow(row: any) {
    ...
    if (this.pickupCartDataList.product.length === 0) return;

    if (this.pickupCartDataList.product[row].tax === undefined) {
      this.pickupCartDataList.product[row].tax = 0;
    }
    else if (this.pickupCartDataList.product[row].tax === null || this.pickupCartDataList.product[row].tax === '') {
      this.pickupCartDataList.product[row].tax = 0;
    }

    let priceWithQty = this.pickupCartDataList.product[row].price * this.pickupCartDataList.product[row].quantity;

    // //@@TODO
    // if (this.pickupCartDataList.product[row].discount===undefined || this.pickupCartDataList.product[row].discount ===null){
    //   this.pickupCartDataList.product[row].discount = 0;
    // }
    // if (this.pickupCartDataList.product[row].discount===undefined || this.pickupCartDataList.product[row].discount ===null){
    //   this.pickupCartDataList.product[row].discount = 0;
    // }
    ...
    if (this.showTaxFlag) {
      if (this.pickupCartDataList.product[row].discount === undefined) {
        this.pickupCartDataList.product[row].discount = 0;
      }

      if (this.pickupCartDataList.product[row].discountVal === undefined) {
        this.pickupCartDataList.product[row].discountVal = 0;
      }

      this.pickupCartDataList.product[row].discountVal = (priceWithQty * (this.pickupCartDataList.product[row].discount)) / 100;

      let priceMinusDiscount = (priceWithQty) - Number(this.pickupCartDataList.product[row].discountVal);
      this.pickupCartDataList.product[row].totalTax = ((priceMinusDiscount) * this.pickupCartDataList.product[row].tax) / 100;
      this.pickupCartDataList.product[row].totalPrice = ((Number(priceMinusDiscount) + this.pickupCartDataList.product[row].totalTax).toFixed(2));
      ...
      this.pickupCartDataList.product[row].totalPrice = ((Number((priceWithQty) - Number(this.pickupCartDataList.product[row].discountVal))).toFixed(2));
    }
  ...
  priceCalculationTotal(): void {
    ...
    this.pickupCartDataList.totalItems = this.pickupCartDataList.product.length;
    ...
    if (this.showTaxFlag) {
      ...
      for (let row = 0; row < this.pickupCartDataList.product.length; row++) {
        let totalTax = 0, totalDiscount = 0, total = 0, grandTotal = 0, totalQty = 0, totalPrice = 0;


        // totalTax = Number(this.pickupCartDataList.product[row].totalTax);
        ...
        // totalDiscount = Number(this.pickupCartDataList.product[row].discountVal);
        // this.priceSummary.discount += Number(totalDiscount);

        totalQty = Number(this.pickupCartDataList.product[row].quantity);
        this.pickupCartDataList.totalQty += Number(totalQty);

        total = Number(this.pickupCartDataList.product[row].price * totalQty);
        this.pickupCartDataList.subTotal += Number(total);

        // totalPrice = Number(this.pickupCartDataList.product[row].price * totalQty);
        // this.priceSummary.totalWithoutDiscount += totalPrice;

        this.pickupCartDataList.total += Number(this.pickupCartDataList.product[row].totalPrice);
      ...
      for (let row = 0; row < this.pickupCartDataList.product.length; row++) {
        ...
        let qty = Number(this.pickupCartDataList.product[row].quantity);
        totalQty = qty;
        this.pickupCartDataList.totalQty += Number(totalQty);

        total = Number(this.pickupCartDataList.product[row].totalPrice);
        this.pickupCartDataList.subTotal += Number(total);
    ...
    this.pickupCartDataList.total = (this.pickupCartDataList.subTotal - this.pickupCartDataList.discount) + this.pickupCartDataList.taxes;
  ...
  calculateDiscount(discount: any, row: any): any {
    ...
    let price = this.pickupCartDataList.product[row].price * this.pickupCartDataList.product[row].quantity;
    let discountVal = 0;
    ...
    this.pickupCartDataList.product[row].discountVal = discountVal;
  ...
  calculateTaxTotal(totalTax: any): any {
    ...
    this.pickupCartDataList.taxesPercentage = totalTax;
    //let netSale = (this.priceSummary.total  - this.priceSummary.discount);
    let netSale = (this.pickupCartDataList.subTotal - this.pickupCartDataList.discount);
    let taxValue = (netSale * totalTax) / 100;

    this.pickupCartDataList.total = netSale + taxValue;
    this.pickupCartDataList.taxes = taxValue;
  ...
  /* *********************************************************************************** */
  printSentToKitchen(orderNum: any, todaydatashow: any) {
    ...
    if (popupWin != null || popupWin != undefined) {
      ...
      <div class="recipt_container">  
        <div class="header"> 
          ...
          <p style="text-align:left"><b> Order#:${orderNum}  </b></p>
          <hr>
      ...
      this.pickupCartDataList.product.forEach((item: any) => {
        //Only add new item
        // if (item.orderItemId === null) {
        bodyHtml += `<p style="text-align:left"> ${item.productName} x ${item.quantity} </p>`;
        // }

      });

      let footerHtml = `
            <p> Special Instructions: ${this.orderNotes} </p>
          </div>
        </div>
     </body>
    </html>
