    ...
    if (localCartData) {
      ...
      if (cartData.product.length > 0) {
        ...
        for (let i = 0; i < cartData.product.length; i++) {
          if (cartData.product[i].upc !==null){
...
@media print {
  ...
  calculateFooterBalance(): void {
    ...
    let discountPercentage = parseFloat(this.totalDiscount) || 0;
    let totalTax = Number(this.totalTax) || 0;
    
    //Calculate Discount Value in Rs
    let cashDiscount = (this.priceSummary.total * discountPercentage)/100;
    ...
    if (this.showTaxFlag) {
      // if (this.priceSummary.tax>0){
      //   this.customerBalance = (this.priceSummary.total - cashDiscount + this.priceSummary.tax);
      // }
      if (totalTax>0)
      {
        let afterDicountAmount = this.priceSummary.total - cashDiscount; 
        this.priceSummary.tax = ( afterDicountAmount * totalTax )/100; 
        this.customerBalance = afterDicountAmount + this.priceSummary.tax;
      }
      else{
...
onNameSearch(event: any) {
  ...
  selectCustomer(customer: Customer) {
    ...
    if (this.customer.discountPercentage===undefined || this.customer.discountPercentage===null){
      ...
      this.totalDiscount = this.customer.discountPercentage;
      this.priceCalculationTotal();
      this.calculateFooterBalance();
    }
