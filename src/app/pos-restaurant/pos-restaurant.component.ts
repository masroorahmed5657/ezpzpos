...
import {faPrint,faList, faCloudUpload, faCloudDownload, faPerson, faCreditCard, faCashRegister, faPlusSquare, faDashboard, faRemove, faRupeeSign, faDollar, faCar, faHome, faSave, faUndo, faFilter, faEdit, faPlusCircle, faHistory, faFileInvoiceDollar, faShoppingCart, faSort, faSearch } from '@fortawesome/free-solid-svg-icons';
        ...
        this.productService.getProductsByUPC(upc).subscribe((data) => {
          if (data === null) {
            ...
            if (productId === null) {
              ...
              rcvdProduct.agentId = this.selectedAgent;
              rcvdProduct.loginId = this.selectedAgent;
...
@media print {
  ...
  priceCalculationPerRow(row: any) {
    ...
    if (this.cartDataList.product[row].tax === undefined) {
      this.cartDataList.product[row].tax = 0;
    }
    else if (this.cartDataList.product[row].tax === null || this.cartDataList.product[row].tax === '') {
      this.cartDataList.product[row].tax = 0;
    }
  ...
  priceCalculationTotal(): void {
    ...
    this.priceSummary.totalItems = this.cartDataList.product.length;
